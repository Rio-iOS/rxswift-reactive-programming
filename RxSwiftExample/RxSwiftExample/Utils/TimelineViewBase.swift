...
private let eventWidth: CGFloat = 40
...
open class TimelineViewBase: UIView {
    ...
    open override func willMove(toSuperview newSuperview: UIView?) {
        ...
        if newSuperview == nil {
            ...
            detachDisplayLink()
            displayLink = CADisplayLink(target: DisplayLinkTarget(view: self), selector: #selector(DisplayLinkTarget.update(_:)))
    ...
    fileprivate func update(_ sender: CADisplayLink) {
        let now = Date()
        let start = now.addingTimeInterval(-11)
        let width = frame.width
        let increment = (width - eventWidth) / 10.0
        ...
        events.forEach { box in
            if let view = box.view {
                ...
                let origin = Int(width - eventWidth + interval * increment)
...
private final class DisplayLinkTarget: NSObject {
    private weak var view: TimelineViewBase?
    init(view: TimelineViewBase) { self.view = view }
    @objc func update(_ displayLink: CADisplayLink) {
        guard let view = view else { displayLink.invalidate(); return }
